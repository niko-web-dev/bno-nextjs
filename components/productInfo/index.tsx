	...
	return (
		<form className={s.card__info}>
					...
					<ul className={s.card__save_blockList}>
						{
							product?.materials.map((item, index) => {
								return (
									<li key={index}><h2>{item.label}:</h2><p>{item.text}</p></li>
								)
							})
						}
					</ul>
			...
			<div className={s.card__description}>
				{product.description.split('<br />').map((str, index) => <p key={index}>{str}</p>)}
			</div>
					...
					? colors.map(({ id, color }) => {
						return (
							<Link href={`/products/${id}`} key={id}>
								<a className={[s.card__colorsLink, id === product.id ? s.card__colorsLinkActive : null].join(' ')}
